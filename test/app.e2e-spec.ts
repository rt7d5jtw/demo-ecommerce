...
describe('AppController (e2e)', () => {
  ...
  let testUser: User;
  // Information about the state changes to clean up
  ...
  beforeAll(async () => {
    ...
    // The reason I storage the user to this variable is because it is being used to
    // test some of the other user endpoints that require that the User has to be the
    // same as the one you're signed in with. This is because the JWT
    // provides the User context.
    testUser = Array.from(res.body).find(
      ({ email }: User) => email === 'testing@user.com'
    ) as User;
  });
