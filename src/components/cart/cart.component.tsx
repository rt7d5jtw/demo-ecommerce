...
const Cart = ({ isOpen, cartItems, quantity}: ICart) => {
  ...
  const handleClickOutside = (event: any) => {
    if (wrappedRef.current && iconRef.current && !iconRef.current.contains(event.target) && !wrappedRef.current.contains(event.target)) {
    ...
    <div className='cart'>
      ...
      { isOpen ?
          ...
          <div className='checkout-cart'>
            ...
            <button className='clear-cart-btn' onClick={() => cartClear()} title="Clears cart from all products">Clear Cart</button>
          </div>
            {cartItems.length ? ( cartItems.map(({ title, price, image, quantity, id }) => (
              <__CartItem key={id} id={id} title={title} price={price} image={image} quantity={quantity}/>
